...
```{r}
tmpdir <- tempdir()
fdir <- file.path(tmpdir, "feather")
dir.create(fdir)
```

```{r}
fpr <- feather_genoprob(pr, "pr", fdir)
fapr <- feather_genoprob(apr, "apr", fdir)
```
...
Two `feather_genoprob` objects using same area. Combine using `cbind`. Notice that the order of chromosomes is reversed by joining `fapr2` to `fapr3`.

```{r}
fdir2 <- file.path(tmpdir, "feather2")
dir.create(fdir2)
fapr2 <- feather_genoprob(subset(apr, chr="2"), "apr", fdir2)
fapr3 <- feather_genoprob(subset(apr, chr="3"), "apr", fdir2)
```
